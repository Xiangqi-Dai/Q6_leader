#!/usr/bin/env python3
"""
left_arm_slot2.py — 左臂电机控制脚本 (不依赖 ROS)

与 left_arm.py (slot1 左臂) 完全同源, 仅默认 CAN 口改为 slot2 左臂 can_slot2_ch0.
...
但完全不走 ROS / ros2_control, 直接用 openarm_can 的 Python 绑定驱动 CAN:
  ...
  setzero —— set_zero_all():   把当前位姿写入电机为零点 (持久; 标定用, 先手掰到位!)
...
slot2 左臂现场接口 = can_slot2_ch0 (slot1 左臂在 left_arm.py).

用法:
    python3 left_arm_slot2.py home                 # 完整: 使能 → 回零 → 失能 (默认)
    python3 left_arm_slot2.py enable               # 仅使能 (抱住当前位, 不运动)
    python3 left_arm_slot2.py zero                 # 仅回零 (需已使能)
    python3 left_arm_slot2.py disable              # 仅失能
    python3 left_arm_slot2.py setzero              # 标定零位: 把当前位姿写入电机为零点(先手掰到位!)
    python3 left_arm_slot2.py home --no-can        # 跳过 CAN 口拉起(已配好时)
    python3 left_arm_slot2.py home --iface can0    # 指定其它 CAN 口 (一般不用)
    python3 left_arm_slot2.py home --no-gripper    # 不带夹爪(只有 7 关节)
    python3 left_arm_slot2.py --help
"""
...
# 现场配置: slot2 左臂 = can_slot2_ch0
DEFAULT_IFACE = "can_slot2_ch0"
...
# ── 阶段 4: 标定零位 (把当前位姿写入电机为零点, 持久; 不使能, 臂可自由手掰到位) ──
def setzero(oa):
...
def run(cmd, iface, with_gripper, do_can, steps, step_ms):
    ...
    try:
        if cmd == "enable":
            ...
            return_to_zero(oa, with_gripper, steps, step_ms)
            disable(oa);           print(f"  [{iface}] 已失能, 左臂回零完成")
