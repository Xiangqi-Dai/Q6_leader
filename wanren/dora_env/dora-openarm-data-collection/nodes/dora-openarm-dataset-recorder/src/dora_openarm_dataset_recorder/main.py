...
class DatasetWriter:
    ...
    def __init__(self, directory, name, metadata):
        ...
        Resume support: never wipes the dataset. Existing episodes are kept,
        the next episode number continues from max(existing)+1, and the prior
        metadata episode list is loaded so it survives across runs.
        """
        self._directory = directory
        self._name = name
        ...
        episodes_dir = self._base_directory / "episodes"
        existing = (
            [int(p.name) for p in episodes_dir.glob("*") if p.name.isdigit()]
            if episodes_dir.exists() else []
        )
        self._next_episode_number = max(existing) + 1 if existing else 0
    ...
    def next_episode_number(self):
        """Return the next episode number, incrementing the counter."""
        number = self._next_episode_number
        self._next_episode_number += 1
        return number

    def _load_existing_episodes(self):
        """Load the episodes list from an existing metadata.yaml (resume)."""
        meta_path = self._base_directory / "metadata.yaml"
        if not meta_path.exists():
            return
        try:
            with open(meta_path, encoding="utf-8") as f:
                data = yaml.safe_load(f) or {}
            self._episode_results = list(data.get("episodes") or [])
        except Exception:
            self._episode_results = []
...
def main():
    ...
    for event in node:
        ...
        if event_id == "command":
            ...
            if command == "start":
                episode = Episode()
                episode.number = dataset_writer.next_episode_number()
