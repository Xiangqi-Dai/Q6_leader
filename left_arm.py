#!/usr/bin/env python3
"""
left_arm.py — 左臂电机控制脚本 (不依赖 ROS)

与 right_arm.py 完全同源, 仅默认 CAN 口改为左臂 can_slot1_ch0.
...
左臂现场接口 = can_slot1_ch0 (与 launch 的 left_can_interface 一致).

用法:
    python3 left_arm.py home                 # 完整: 使能 → 回零 → 失能 (默认)
    python3 left_arm.py enable               # 仅使能 (抱住当前位, 不运动)
    python3 left_arm.py zero                 # 仅回零 (需已使能)
    python3 left_arm.py disable              # 仅失能
    python3 left_arm.py setzero              # 标定零位: 把当前位姿写入电机为零点(先手掰到位!)
    python3 left_arm.py home --no-can        # 跳过 CAN 口拉起(已配好时)
    python3 left_arm.py home --iface can0    # 指定其它 CAN 口 (一般不用)
    python3 left_arm.py home --no-gripper    # 不带夹爪(只有 7 关节)
    python3 left_arm.py --help
"""
...
# 现场配置: 左臂 = can_slot1_ch0 (与 launch 的 left_can_interface 一致)
DEFAULT_IFACE = "can_slot1_ch0"
