...
任意 CAN 口 (slot2 等) -- 直接 --can 指定接口:
    python3 enable_openarm.py enable  --can can_slot2_ch0 can_slot2_ch1   # 使能 slot2 两臂
    python3 enable_openarm.py disable --can can_slot2_ch0 can_slot2_ch1   # 失能 slot2 两臂
    python3 enable_openarm.py enable  --can can_slot2_ch0                 # 单口
"""
import sys
import time
import subprocess
...
def main():
    ...
    argv = sys.argv[1:]
    # --can IFACE [IFACE ...]: 直接指定任意 CAN 口(如 slot2), 覆盖 left/right
    cans = []
    if "--can" in argv:
        i = argv.index("--can")
        j = i + 1
        while j < len(argv) and not argv[j].startswith("-") and argv[j] not in ("enable", "disable"):
            cans.append(argv[j]); j += 1
        argv = argv[:i] + argv[j:]
    args = [a for a in argv if a != "--no-can"]
    ...
    if cans:
        targets = {c: c for c in cans}            # name = iface
    else:
        ...
        if side == "both":
            ...
            print(f"未知臂: {side} (left / right / both) 或用 --can <接口>"); return
