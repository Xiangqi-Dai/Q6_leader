...
右臂现场接口 = can_slot1_ch1 (与 launch 的 right_can_interface 一致).

用法:
    python3 right_arm.py home                 # 完整: 使能 → 回零 → 失能 (默认)
    python3 right_arm.py enable               # 仅使能 (抱住当前位, 不运动)
    python3 right_arm.py zero                 # 仅回零 (需已使能)
    python3 right_arm.py disable              # 仅失能
    python3 right_arm.py setzero              # 标定零位: 把当前位姿写入电机为零点(先手掰到位!)
    python3 right_arm.py home --no-can        # 跳过 CAN 口拉起(已配好时)
    python3 right_arm.py home --iface can0    # 指定其它 CAN 口
    python3 right_arm.py home --no-gripper    # 不带夹爪(只有 7 关节)
    python3 right_arm.py --help
"""
...
# 现场配置: 右臂 = can_slot1_ch1 (与 launch 的 right_can_interface 一致)
DEFAULT_IFACE = "can_slot1_ch1"
...
cd /ros2_ws
python3 right_arm.py home              # 使能 → 回零 → 失能  (问题要的三步,默认动作)
python3 right_arm.py enable            # 仅使能(抱住当前位,不动)
python3 right_arm.py zero              # 仅回零(需已使能)
python3 right_arm.py disable           # 仅失能
python3 right_arm.py home --no-can     # CAN 口已配好时跳过 ip link
python3 right_arm.py home --iface can0 # 换 CAN 口
python3 right_arm.py home --no-gripper # 只有 7 关节,无夹爪
